...
features_matrix = numpy.asarray([embed[numpy.where(embed[:,0]==node), 1:] for node in range(len(graph))])
...
#    assert sum(len(l) for l in y_train) == y_train_.nnz
    ...
    for average in averages:
        results[average] = f1_score(y_test_onehot, label2onehot(preds, 39), average=average)
