...
function [X Y] = CorrelationDecomp( M, dim )
...
obj_fun = @(X,Y)  norm( M - X*Y', 'fro')^2  + norm(Y - X - Mu./lambda, 'fro')^2*lambda/2;
Y_gradient = @(X,Y) -2.*(M-X*Y')'*X + lambda.*(Y- X-Mu./lambda);
X_gradient = @(X,Y) -2.*(M-X*Y')*Y - lambda.*( Y-X-Mu./lambda );
